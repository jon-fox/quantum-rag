...
class ClassicalReranker:
    ...
    def _initialize_model(self):
        ...
        if os.path.isdir(local_model_dir):
            try:
                ...
                logger.info(
                    f"Loaded Cross-Encoder model from local cache: {local_model_dir}"
                )
                return
            except Exception as e:
                logger.warning(
                    f"Failed to load local model from {local_model_dir}: {e}. Proceeding to download."
                )
        ...
        try:
            ...
            try:
                ...
                logger.info(
                    f"Saved Cross-Encoder model to local cache: {local_model_dir}"
                )
            except Exception as e:
                logger.warning(f"Failed to save model to local cache: {e}")
            self.model_loaded = True
            logger.info(
                f"Successfully loaded Cross-Encoder model '{self.model_name}' on device '{self.device}'"
            )
        except Exception as e:
            ...
            for fallback_model in fallback_models:
                try:
                    ...
                    self.model_loaded = True
                    logger.info(
                        f"Successfully loaded fallback Cross-Encoder model '{fallback_model}' on device '{self.device}'"
                    )
                    return
    ...
    def _predict_with_retries(self, inputs: List[Tuple[str, str]]) -> np.ndarray:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.debug(
                    f"Cross-Encoder prediction completed in {elapsed_time:.2f}s for {len(inputs)} pairs"
                )
                return scores
    ...
    def _handle_reranker_failure(
        ...
        logger.error(
            "Cross-Encoder reranking failed - returning original document order with neutral scores"
        )
        logger.error(
            f"Failed to rerank {len(documents)} documents for query: {query[:100]}..."
        )
    ...
    def rerank(
        ...
        if not self._validate_inputs(query, documents):
            logger.error(
                "Input validation failed - returning original documents with neutral scores"
            )
            return [(doc, 0.5) for doc in documents]
                    ...
                    for i, (doc, score) in enumerate(zip(uncached_docs, all_scores)):
                        if self.enable_cache:
                            cache_key = self._get_cache_key(
                                query, self._sanitize_text(doc.content)
                            )
        ...
        elapsed_time = time.time() - start_time
        logger.info(
            f"Reranking completed in {elapsed_time:.2f}s for {len(documents)} documents"
        )
